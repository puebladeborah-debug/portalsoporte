...
type Llamada = { id: string; ownerUid: string; contacto: string; fecha: string; hora: string; notas: string; createdAt: string }
    ...
    <div>
      ...
      {items.length === 0 ? (
        ...
        <div className="space-y-2">
          {pendientes.map(p => (
            <button key={p.id} onClick={() => update(p.id, { hecho: true })}
          ...
          {hechos.map(p => (
            <button key={p.id} onClick={() => update(p.id, { hecho: false })}
              className="w-full flex items-center gap-3 px-4 py-3 rounded-2xl text-left transition-all"
        ...
        <div className="space-y-2">
          {notas.map(n => (
            <NotaCard key={n.id} nota={n}
              onSave={(id, texto) => update(id, { texto, updatedAt: new Date().toISOString() })}
              onDelete={remove} />
          ))}
        </div>
      )}
...
function TabLlamadas({ uid }: { uid: string }) {
  ...
  async function crear() {
    ...
    await add({ ownerUid: uid, contacto: contacto.trim(), fecha, hora, notas: notas.trim(), createdAt: new Date().toISOString() })
    setContacto(''); setFecha(''); setHora(''); setNotas('')
  }
        ...
        <div className="space-y-2">
          {llamadas.map(l => (
            <div key={l.id} className="flex items-center gap-3 px-4 py-3 rounded-2xl" style={{ background: S.card, border: `1px solid ${S.border}` }}>
              ...
              <button onClick={() => remove(l.id)} style={{ color: S.silverDim }}><Trash2 size={14} /></button>
            </div>
          ))}
        </div>
      )}
